    ...
    public void onButtonPress(InventoryClickEvent event) {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                player.setMetadata("CLICKED_BUTTON",new FixedMetadataValue(hHub.getInstance(),player.getUniqueId().toString()));
                ...
                button.clicked(player,event.getSlot(),event.getClick(), event.getView());
                ...
                if (event.isCancelled()) {
                    hHub.getInstance().getServer().getScheduler().runTaskLater(hHub.getInstance(), player::updateInventory, 1L);
                }

                player.removeMetadata("CLICKED_BUTTON", hHub.getInstance());
